...
class cls_image
{
     ...
     * @return  string   错误信息
     */
    function add_watermark_word($dst_path,$words,$thumb=0,$newname='_watermark')
    {
        $dst_path=ROOT_PATH.$dst_path;
        $file_info=pathinfo($dst_path);
        $save_path=str_replace($file_info['filename'], $file_info['filename'].'_watermark', $dst_path);
        print_r($save_path);exit;
        $dst = imagecreatefromstring(file_get_contents($dst_path));
        //打上文字
        $font = ROOT_PATH.'data/captcha/arial.ttf';//字体路径
        $black = imagecolorallocate($dst, 0x00, 0x00, 0x00);//字体颜色
        //缩略图位置
        if($thumb)
            imagefttext($dst, 14, 0, 30, 30, $black, $font, $words);
        else
            imagefttext($dst, 38, 0, 42, 80, $black, $font, $words);
     ...
     * @return  string   覆盖原图
     */
    function add_watermark_word2($dst_path,$words,$thumb=0)
    {
        $dst_path=ROOT_PATH.$dst_path;
        //创建图片的实例
        $dst = imagecreatefromstring(file_get_contents($dst_path));
