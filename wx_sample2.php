...
require( ROOT_PATH . '/class/wechat.class.php');
require( ROOT_PATH . '/function.php');
require_once(ROOT_PATH. '/class/cls_log.php');
$config = array(
    'type'=>'file',
...
function do_text($weObj){
        ...
        if(substr($keyword,0,6) == '天气'){
            ...
            $keyword = strreplace($keyword);
            $temp= songs_baidu(str_replace("点歌","",$keyword));
            ...
            // }
            if(is_array($temp)){
                $weObj->music($temp['name'],round($temp['file_size']/(1024*1024),2).'M',$temp['file_link'],$temp['file_link'])->reply();
                exit;
            }else{
                $content=$temp;
            }
        }elseif(substr($keyword,0,6) == '歌词'){
            $keyword = strreplace($keyword);
            $content = songs_baidu(str_replace("歌词","",$keyword),1);
        ...
        //     $content = c1c();
            ...
            if($result){
                ...
                $content = $result;
        }elseif(substr($keyword,0,6) == '搜索'){
            require_once(ROOT_PATH. '/class/QueryList/QueryList.class.php');
            // 搜索笑话-1-2
            ...
            $url = "http://storebt.org/s/$temp[0]/0/0/$pages.html";
                $GLOBALS['logger']->writeLog(print_r($url,1),'INFO','test');
...
function mylog($log)
{
    ...
    $postObj= xmlToArray($log);
    switch ($postObj['MsgType']) {
        case 'text':
            $content=$postObj['Content'];
            break;
        case 'music':
            $content=var_export($postObj['Music'],1);
            break;
        case 'event':
            $content=$postObj['EventKey'];
            break;
        case 'news':
            $content=var_export($postObj['Articles'],1);
            break;
        default:
            $content=var_export($postObj,1);
            break;
    }
    $GLOBALS['logger']->writeLog($postObj['FromUserName'].':'.$content,'INFO');
    // error_log( date('Y-m-d H:i:s',time()).' '.var_export($GLOBALS['logger'],true)." \n", 3, "testssss.txt");
    // error_log( date('Y-m-d H:i:s',time()).' '.$postObj['FromUserName'].':'.$content." \n", 3, "log.txt");
}
/**
 * XML文档转为数组
